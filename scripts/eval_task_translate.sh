...
do
    ...
    do
        ...
        DATA_DIR="${LOCAL_DATA_PATH}eval_traces/${SHOW_TASK_NAME}+en/output.jsonl"
        FULL_TASK_NAME="${TASK}translate"
        yeval \
            --model ${MODEL_PATH}${MODEL}${MODEL_SUFFIX} \
            --sample_args "n=1,temperature=0.7,logprobs=True,top_p=0.8,extra_body={'top_k': 20}" \
            --task "${FULL_TASK_NAME}t//${PROMPT}" \
            --data_kwargs "{'data_files': '${DATA_DIR}'}" \
            --include_path lang_boot/tasks/ \
            --api_base "http://localhost:${PORT}/v1" \
            --run_name $MODEL+$SHOW_TASK_NAME+translate-test \
            --trust_remote_code \
